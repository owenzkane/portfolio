// First, let's prepare some data for crunching vertices, faces, textures, 
// colors and normals stored by PLY file format.
...
// Now let's define LoadPLY function that will load the PLY model filename from a URL location, 
// parse the data and return it in separate arrays, ready for passing onto WebGL VBO attribute 
// functions just as explained in other examples on the site.

// Load PLY function;
var xmlhttp;
function LoadPLY(filename)
{
	//resetting all global variables for loading new model data
	ReadingPLYData = false;
	PLY_Vertices = 0;
	PLY_Faces = 0;
	FaceIndex = 0;
	VAO_VertexIndex = 0;
		...
		if (xmlhttp.readyState == XMLHttpRequest.DONE) {
			if (xmlhttp.status == 200) {
				...
				for (var i = 0; i < arrayTexture.length/2; i++){
					//                    	texU += modelData[2][2*i+0];
					//                  	texV += modelData[2][2*i+1];
